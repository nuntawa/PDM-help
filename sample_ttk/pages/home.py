...
def Home(container,frames):
    ...
    home_label.grid(row=0,column=0,padx=5,pady=5,sticky="NEWS")
    home_label2 = ttk.Label(page, text="Home Page", font=("Arial", 10))
    home_label2.grid(row=0,column=1,padx=5,pady=5,sticky="NEWS")

    change_page_btn = ttk.Button(page, text="Go to Profile", command=lambda: frames["profile"].tkraise() )
    change_page_btn.grid(row=1,column=0,padx=5,pady=5,sticky="NEWS")
    
    # text_area = ttk.Text(page,height=180,width=129)
    # text_area.grid(row=2,column=0,columnspan=12,padx=5,pady=5,sticky="NEWS")

    st = ScrolledText(page, height=20,width=145, autohide=True)
    st.grid(row=2,column=0,columnspan=12,padx=5,pady=5,sticky="NEWS")
    ...
    clear_btn.grid(row=3,column=0,padx=5,pady=5,sticky="NEWS")
